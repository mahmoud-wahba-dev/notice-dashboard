...
window.addEventListener('load', () => {
  const dataTable = new HSDataTable('#datatable-roles', {
    columnDefs: [
      {
        ...
        render: function (data, type, full, meta) {
          ...
          return `<div class="flex gap-3">
              ...
              <div class="flex flex-col">
                <div class="text-nowrap text-base-content font-medium hover:text-primary">${name}</div>
  ...
  function applyFilters() {
    ...
    const rows = document.querySelectorAll('#datatable-roles tbody tr')
    rows.forEach(row => {
      const roleCell = row.querySelector('td:nth-child(3)')
      const planCell = row.querySelector('td:nth-child(4)')
      if (!roleCell || !planCell) return
  ...
  const userTable = document.querySelector('#datatable-roles')
  if (userTable) {
  ...
  const dtSelectTable = document.querySelector('#datatable-roles')
  if (dtSelectTable) {
